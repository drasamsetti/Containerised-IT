...
To create the image `drasamsetti/mysql`, execute the following command on the mysql folder:

        docker build -t drasamsetti/mysql 5.6/

To run the image and bind to port 3306:

        docker run -d -p 3306:3306 drasamsetti/mysql
...
set the environment variable `MYSQL_PASS` to your specific password when running the container:
        ...
        docker run -d -p 3306:3306 -e MYSQL_PASS="mypass" drasamsetti/mysql
...
that names the database.
        ...
        docker run -d -p 3306:3306 -e ON_CREATE_DB="newdatabase" drasamsetti/mysql
...
on the container to store the database files. To do so:
        ...
        docker run -d -v /path/in/host:/var/lib/mysql drasamsetti/mysql /bin/bash -c "/usr/bin/mysql_install_db"
...
After this you can start your MySQL image, but this time using `/path/in/host` as the database folder:
        ...
        docker run -d -p 3306:3306 -v /path/in/host:/var/lib/mysql drasamsetti/mysql
...
To do so, first create a container that holds database files:
    ...
    docker run -d -v /var/lib/mysql --name db_vol -p 22:22 drasamsetti/ubuntu-trusty
...
After this you can start your MySQL image using volumes in the container created above (put the name of container in `--volumes-from`)
    ...
    docker run -d --volumes-from db_vol -p 3306:3306 drasamsetti/mysql
...
To import a SQL backup which is stored for example in the folder `/tmp` in the host, run the following:
        ...
        sudo docker run -d -v /tmp:/tmp drasamsetti/mysql /bin/bash -c "/import_sql.sh <user> <pass> /tmp/<dump.sql>"

Also, you can start the new database initializing it with the SQL file:

        sudo docker run -d -v /path/in/host:/var/lib/mysql -e STARTUP_SQL="/tmp/<dump.sql>" drasamsetti/mysql
...
- Master MySQL
-
        docker run -d -e REPLICATION_MASTER=true -e REPLICATION_PASS=mypass -p 3306:3306 --name mysql drasamsetti/mysql

- Example on Slave MySQL:
-
        docker run -d -e REPLICATION_SLAVE=true -p 3307:3306 --link mysql:mysql drasamsetti/mysql
...
- Volume created by MySQL 5.6 cannot be used in MySQL 5.5